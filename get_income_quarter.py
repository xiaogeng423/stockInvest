...
from crawler_defs import get_co_id_name_type, get_income_byco
...
# get n_periods seasonal income statement
path_new = path + 'stockInvest/tab_income/'
if os.path.isdir(path_new) == False:
...
for co_info in co_infos.itertuples():
    ...
    while i < n_periods:
        ...
        while True:
            try:
                df = get_income_byco(co_info.co_id, year, season)
                df.to_csv(path_new + 'income_' + '-'.join([str(e) for e in [co_info.co_id, co_info.co_name, year, season]]) + '.csv', encoding='utf_8_sig')
                i += 1
                break
            except Exception as e:
                errMsg = get_traceback(e)
